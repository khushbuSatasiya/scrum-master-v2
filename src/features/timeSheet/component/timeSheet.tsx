...
import { Box, Button, Flex, Space, Text, ThemeIcon, Tooltip } from '@mantine/core';
...
const TimeSheet: FC<IUserTimeSheetProps> = ({ handleTimeSheetDetails, uId }) => {
	const navigate = useNavigate();
	const [isLoading, setLoading] = useState(false);
	const [value, setValue] = useState<Date | null>(new Date());
	const [timeSheetData, setTimeSheetData] = useState<[]>([]);
	const [task, setTask] = useState({} as { plannedTasks: string; completedTasks: string });
	...
	const getUerTimeSheet = useCallback(
		(date?: any) => {
			const params = {
				startDate: date ? moment(date).startOf('month').format('YYYY-DD-MM') : '',
				endDate: date ? moment(date).endOf('month').format('YYYY-DD-MM') : ''
			};
			setLoading(true);
			httpService
				.get(`${API_CONFIG.path.getTimeSheet}`, params)
				.then((res) => {
	...
	const getTimeSheetExcel = useCallback(() => {
		const params = {
			startDate: value ? moment(value).startOf('month').format('YYYY-DD-MM') : '',
	...
	const onClose = useCallback(() => setTask({} as { plannedTasks: string; completedTasks: string }), []);
		...
		>
			<Flex justify='space-between'>
				...
				<Tooltip label='Download Excel' color='blue' position='bottom' withArrow>
					<ThemeIcon w={40} h={40} onClick={getTimeSheetExcel} mb={15} radius='md'>
