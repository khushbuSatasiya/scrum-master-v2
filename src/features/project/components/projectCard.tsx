...
import { Avatar, Box, Flex, Loader, Menu, Paper, Text, Tooltip, createStyles } from '@mantine/core';
import { IconDotsVertical, IconFileDownload, IconUserCircle, IconCalendarEvent } from '@tabler/icons-react';
...
interface IProjectCardProps {
	...
	uId: string;
	isLoading: boolean;
	getTeamReport: (projectId: string, month: string, projectName: string) => void;
...
const ProjectCard: FC<IProjectCardProps> = ({
	...
	return (
		<Flex align={'center'} wrap={'wrap'} gap={30} justify={`${isLoading && 'center'}`}>
			{isLoading && (
				<Loader
					variant='dots'
					sx={{
			...
			{!isEmpty(projectInfo) &&
				!isLoading &&
				projectInfo.map(({ projectId, projectName, teamDetails, date, logo, leadDetails }, index) => {
					return (
						<Paper
							shadow='sm'
							p={20}
							...
							<Fragment key={projectId}>
								...
								<Flex>
									<Avatar.Group spacing='xs' sx={{ cursor: 'pointer' }}>
										{teamDetails.map(({ avatar, realName, id }, index: number) => {
											return (
												<Box key={id}>
													{index < 8 && (
														<Tooltip
